...
loss.matrix <- matrix(nrow=500, ncol=500)
loss.var95 <- rep(0,500)
loss.avar95 <- rep(0,500)
loss.var99 <- rep(0,500)
loss.avar99 <- rep(0,500)
...
for (i in 1:500) {
  ...
  for (j in 1:500) {
    ...
    loss.matrix[i,j] <- total.loss
  }
  
  # Calculate VaR for both confidence levels
  curr.loss.sum <- loss.matrix[i,]
  curr.var <- quantile(curr.loss.sum, probs = seq(0.95, 0.99))
  
  loss.var95[i] <- curr.var[1]
  loss.var99[i] <- curr.var[2]
  
  # Calculate AVaR for both confidence levels
  avar.vals95 <- curr.loss.sum[curr.loss.sum>=curr.var[1]]
  ## NEED TO CALCULATE INTEGRAL
  
  avar.vals99 <- curr.loss.sum[curr.loss.sum>=curr.var[2]]
  ## NEED TO CALCULATE INTEGRAL
}

## REPEAT FOR FIVE YEAR LOSSES
```
