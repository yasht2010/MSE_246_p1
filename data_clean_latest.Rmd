...
# Quick visualization
tr.sampl.rows <- sample(1:nrow(loss_train), 10000, replace=FALSE)
tr.sampl <- loss_train[tr.sampl.rows,]
par(mar=c(1,1,1,1))
pairs(tr.sampl[,c(5,7:10,15,17,19,21,24)])
...
for (i in 1:nrow(loss_valid)) {
  if (is.na(loss_valid$GrossChargeOffAmount[i]) | is.na(loss.pred[i])) {
    if (rem.rows[length(rem.rows)]==0) {
      rem.rows[length(rem.rows)] <- i
    } else {
      length(rem.rows) <- length(rem.rows)+1
      rem.rows[length(rem.rows)] <- i
    }
  }
}

loss_valid <- loss_valid[-rem.rows,]
loss.pred <- loss.pred[-rem.rows]
