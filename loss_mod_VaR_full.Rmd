...
def.probs <- read.csv("prob.csv") # --> generate random probabilities to make sure procedure works
...
test.loss <- predict(bag.tree, newdata=test500_set, type="response")
...
# Cleaning NA values for 1-year test set
test500_set <- test500.5[!is.na(test500.5$interestRate),]
test500_set <- test500.5[!is.na(test500.5$unemploymentRate),]
test500_set <- test500.5[!is.na(test500.5$hpiState),]
test500_set <- test500.5[!is.na(test500.5$tedSpread),]
test500_set <- test500.5[!is.na(test500.5$sandp500),]
test500_set <- test500.5[!is.na(test500.5$TermInMonths),]

# Setting correct default events for 5-year horizon
for (i in 1:nrow(test.def)) {
  val <- test.def$loanNum1[i]
  test500.5[test500.5$loanNum1==val,24] <- 1
}
...
for (i in 1:10000) {
  ...
  def.loss <- test.loss * test500.1$GrossApproval * def.events
...
for (i in 1:10000) {
  ...
  def.loss <- test.loss * test500.5$GrossApproval * def.events
...
np.junior.tranche.1 <- ifelse(np.lossvector.1 > np.thresh5.1, min(c(100, (np.lossvector.1-np.thresh5.1)/np.thresh15.1)), 0)
np.senior.tranche.1 <- ifelse(np.lossvector.1 > np.thresh15.1, (np.lossvector.1-np.thresh15.1)/maxloss, 0)

np.junior.plot.1 <- hist(np.junior.tranche.1)
np.senior.plot.1 <- hist(np.senior.tranche.1)
...
np.junior.tranche.5 <- ifelse(np.lossvector.5 > np.thresh5.5, min(c(100, (np.lossvector.5-np.thresh5.5)/np.thresh15.5)), 0)
np.senior.tranche.5 <- ifelse(np.lossvector.5 > np.thresh15.5, (np.lossvector.5-np.thresh15.5)/np.maxloss, 0)

np.junior.plot.5 <- hist(np.junior.tranche.5)
np.senior.plot.5 <- hist(np.senior.tranche.5)
