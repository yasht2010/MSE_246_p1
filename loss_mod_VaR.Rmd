...
loss_train <- loss_train[!is.na(loss_train$SBLR),]
...
loss_valid <- loss_valid[!is.na(loss_valid$SBLR),]
...
loss.lm <- lm(LossRatio~interestRate+unemploymentRate+hpiState+tedSpread+sandp500+TermInMonths, data=loss_train,na.action=na.exclude)
summary(loss.lm)
...
bag.tree <- randomForest(LossRatio~interestRate+unemploymentRate+hpiState+tedSpread+sandp500+TermInMonths, data=loss_train, mtry=8, ntree=30)
summary(bag.tree)
...
rf.tree <- randomForest(GrossChargeOffAmount~interestRate+unemploymentRate+hpiState+tedSpread+sandp500+TermInMonths, data=loss_train, mtry=2, ntree=30)
summary(bag.tree)
...
loss.spl <- lm(LossRatio~bs(interestRate,df=3)+bs(unemploymentRate,df=3)+bs(hpiState,df=3)+bs(tedSpread,df=3)+bs(sandp500,df=3)+bs(TermInMonths,df=3),data=loss_train)
...
# Plot VaR & AVaR
pl.var95.1 <- hist(loss.var95.1)
pl.var99.1 <- hist(loss.var99.1)
pl.avar95.1 <- hist(loss.avar95.1)
pl.avar99.1 <- hist(loss.avar99.1)
...
# Plot VaR & AVaR
pl.var95.5 <- hist(loss.var95.5)
pl.var99.5 <- hist(loss.var99.5)
pl.avar95.5 <- hist(loss.avar95.5)
pl.avar99.5 <- hist(loss.avar99.5)
