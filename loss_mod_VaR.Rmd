...
# Plot VaR Values
ggplot() + geom_density(mapping=aes(x=loss.var95.1))
...
loss.matrix.5 <- matrix(nrow=1000, ncol=1000)
...
for (i in 1:1000) {
  ...
  def.total.loss <- sum(def.loss)
  loss.matrix.5[1,i] <- def.total.loss
}
...
# Bootstrapping
for (i in 2:1000) {
  boot.loss.cols <- sample(1:1000, 1000, replace=TRUE)
