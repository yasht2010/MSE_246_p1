...
set.seed(1)
matrix_test=matrix(0,nrow=0,ncol=22)
finalMatrix=matrix(0,nrow=0,ncol=22)
tempRow = matrix(0,nrow=1,ncol=22)
...
cox_reg=coxph(Surv(start,stop,isDefault)~GrossApproval+unemploymentRate+sandp500+hpiState+
                BusinessType+TermInMonths+SBLR,
              data=training_set)

pred_cox_model= predictProb.coxph(cox_reg,Surv(test_set$start,test_set$isDefault),test_set,2010)

pred = predict(cox_reg,test_set,type="expected")


log_reg=glm(isDefault ~ GrossApproval+hpiState+TermInMonths+SBLR,family=binomial,data=training_set)
